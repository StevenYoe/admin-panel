<!--
    ...
    - Displays a list of all job positions in a table format
    - Super Admins can add, edit, or delete positions
...
@section('title', 'Jabatan - Pazar User Admin')

@section('page-title', 'Jabatan')

@section('content')
    <!-- Header section with page title and Add Position button (for Super Admin) -->
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-xl font-semibold">Daftar Jabatan</h2>
        @if($isSuperAdmin)
        <x-button href="{{ route('positions.create') }}" variant="primary">
    ...
    <!-- Card container for the position table -->
    <x-card>
        @if(count($positions) > 0)
            <div class="overflow-x-auto">
            <!-- Table displaying position data -->
            <x-table 
                :headers="[
                    ['name' => 'ID', 'key' => 'pos_id'],
                    ['name' => 'Kode', 'key' => 'pos_code'],
                    ['name' => 'Nama', 'key' => 'pos_name'],
                    ['name' => 'Status', 'key' => 'pos_is_active']
                ]"
                :sortBy="$sortBy"
                :sortOrder="$sortOrder"
            >
                    ...
                    <tr class="border-b dark:border-gray-700 hover:bg-gray-600">
                        ...
                        <td class="px-5 py-4 text-center">
                            ...
                            <div class="flex justify-center space-x-2">
                                ...
                                @if($isSuperAdmin)
                                <a href="{{ route('positions.edit', $position['pos_id']) }}" class="text-yellow-500 hover:text-yellow-700">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        @endif
        @else
            <!-- Message and button if no positions exist -->
            <div class="py-8 text-center">
                <p class="text-gray-400">Belum ada jabatan yang ditambahkan</p>
                <x-button href="{{ route('positions.create') }}" variant="primary" class="mt-4">
                    Tambah Jabatan
                </x-button>
            </div>
        @endif
